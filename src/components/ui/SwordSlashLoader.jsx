...
export default function SwordSlashLoader({ onComplete, forceTrigger = 0 }) {
  ...
  // Function to trigger slash animation
  ...
  useEffect(() => {
    window.addEventListener("trigger-katana-slash", triggerSlashSequence);
    return () => {
      window.removeEventListener("trigger-katana-slash", triggerSlashSequence);
    };
  }, []);
  ...
  // Handle animation timeline sequence per trigger (key)
  useEffect(() => {
    setStage("slash");
